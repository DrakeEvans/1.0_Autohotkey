...
![::
	...
	try sourceCellPrecedent.select
	ObjRelease(xl)
return
...
!]::
	...
	try sourceCellDependent.select
	ObjRelease(xl)
return
...
!w::
	xl := ComObjActive("Excel.Application")
	xl.CommandBars.ExecuteMso("ShapeWidth")
...
+#Space::
	...
	if (lcDict[(tabName)] > 1) {
		...
		Loop, 10000 {
			if (xl.ActiveSheet.Cells(checkRow, A_Index).Formula = "|") {
				lcDict.Push(Object("check", "valuecheck"))
				lastColumn := lcDict["check"]
				;msgbox found last column
				msgbox, %lastColumn%
				msgBox, % lcDict["test"]
				maxIndex := lcDict.MaxIndex()
				msgbox, max %maxIndex%
				Break
			} else {
	...
	InsertRow:
	;msgbox %checkAddress%
	msgBox chkRow is %checkRow% lcDICt is %lastColumn%
...
^!f::
	...
	xl := ComObjActive("Excel.Application")
	try {
	xl.CommandBars.ExecuteMso("PasteFormulasAndNumberFormatting")
	} catch {
	global copyrange
	copyrange.copy
	xl.CommandBars.ExecuteMso("PasteFormulasAndNumberFormatting")
